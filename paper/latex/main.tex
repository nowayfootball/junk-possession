...
difference ($r=-0.51$); team-matches above 55\% possession with below-median
efficiency average 0.62 goals and 1.00 points versus 2.24 and 2.24 for
...
($p<10^{-4}$) while VAEP itself is not significant --- the index adds
...
distinction on-ball possession value cannot make.
\end{abstract}
...
remains a significant negative predictor ($\beta=-0.020$, $p=0.017$) while
field tilt is not significant ($p=0.14$); regressing xG difference, the
...
while VAEP does not (Table~\ref{tab:vaep}): \junk{} predicts points at
$p<10^{-4}$ with VAEP not significant ($p=0.34$), and efficiency predicts xG
...
event-side junk definition ($q<0.15$); SCI does not overturn that label but
subdivides the windows into these three spatial subtypes.
...
matches (21 windows; 71/24/5) to nine (31; 74/19/6): most flagged junk is indeed
non-space-creating, and the spatial layer earns its keep on the minority that
receive a different spatial verdict.
